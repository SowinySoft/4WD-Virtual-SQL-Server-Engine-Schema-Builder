...
function generateSchemaDDL(schema, engine) {
  ...
  if (schema.comment && engine === 'postgresql') {
    stmts.push(`COMMENT ON SCHEMA ${name} IS ${T.quoteString(schema.comment)};`);
  } else if (schema.comment && engine === 'mssql') {
    stmts.push(`EXEC sp_addextendedproperty @name=N'MS_Description', @value=${T.quoteString(schema.comment)}, @level0type=N'SCHEMA', @level0name=${T.quoteString(name)};`);
  }
  return stmts;
}
...
function generateTableDDL(table, engine) {
  ...
  // Constraints
  for (const c of table.constraints || []) {
    const cDef = constraintDDL(c, table.name, engine, table.schema);
    if (cDef) lines.push(',\n  ' + cDef);
  }
  ...
  // ── Comments (PG/MSSQL) ──
  if (table.comment && engine === 'postgresql') {
    stmts.push(`COMMENT ON TABLE ${fullName} IS ${T.quoteString(table.comment)};`);
  } else if (table.comment && engine === 'mssql') {
    stmts.push(`EXEC sp_addextendedproperty @name=N'MS_Description', @value=${T.quoteString(table.comment)}, @level0type=N'SCHEMA', @level0name=${T.quoteString(table.schema || 'dbo')}, @level1type=N'TABLE', @level1name=${T.quoteString(table.name)};`);
  }
  ...
  for (const col of table.columns) {
    ...
    if (engine === 'postgresql') {
      ...
      stmts.push(`EXEC sp_addextendedproperty @name=N'MS_Description', @value=${T.quoteString(col.comment)}, @level0type=N'SCHEMA', @level0name=${T.quoteString(table.schema || 'dbo')}, @level1type=N'TABLE', @level1name=${T.quoteString(table.name)}, @level2type=N'COLUMN', @level2name=${T.quoteString(col.name)};`);
    }
  }
...
// ─── Constraint DDL ───────────────────────────────────────────────────
function constraintDDL(c, tableName, engine, tableSchema) {
  ...
  switch (c.type) {
    ...
    case 'FOREIGN KEY': {
      ...
      const refSchema = c.refSchema || tableSchema || null;
      const refFullName = refSchema ? `${T.quoteIdent(refSchema, engine)}.${T.quoteIdent(c.refTable, engine)}` : T.quoteIdent(c.refTable, engine);
      let sql = `${cName}FOREIGN KEY (${cols}) REFERENCES ${refFullName} (${refCols})`;
...
function generateViewDDL(view, engine) {
  ...
  } else if (engine === 'mssql' && view.materialized) {
    let sql = `CREATE VIEW ${fullName} WITH SCHEMABINDING AS\n`;
    sql += view.selectQuery;
    stmts.push(sql + ';');
    stmts.push(`-- MSSQL indexed view: create a unique clustered index on ${fullName} to materialize`);
  } else {
    let sql = `CREATE VIEW ${fullName} AS\n`;
    sql += view.selectQuery;
    if (view.checkOption) {
